...
const pt = (e) => ({ lat: e.lat ?? e.center?.lat, lon: e.lon ?? e.center?.lon, t: e.tags });
...
const rest = pts.filter((p) => p.t.highway === "rest_area" || p.t.highway === "services");
console.log(`South-coast Iceland corridor: fuel=${fuel.length} EV=${ev.length} rest=${rest.length}\n`);
...
const hav = (a, b) => {
  ...
  const h = Math.sin(dLat / 2) ** 2 + Math.cos(d(a.lat)) * Math.cos(d(b.lat)) * Math.sin(dLon / 2) ** 2;
...
for (let i = 1; i < ordered.length; i++) {
  ...
  if (g > maxGap) { maxGap = g; at = [ordered[i - 1], ordered[i]]; }
}
console.log(`  stations ordered W->E: ${ordered.length}`);
console.log(`  fuel_gap_max_km = ${maxGap.toFixed(1)} km`);
if (at) {
  console.log(`  between ${(at[0].t.name ?? at[0].t.brand ?? "(unnamed)")} @ ${at[0].lat.toFixed(3)},${at[0].lon.toFixed(3)}`);
  console.log(`      and ${(at[1].t.name ?? at[1].t.brand ?? "(unnamed)")} @ ${at[1].lat.toFixed(3)},${at[1].lon.toFixed(3)}`);
  console.log(`  warning_text would read: "no fuel ${Math.round(maxGap)} km after ${at[0].t.name ?? at[0].t.brand ?? "the last station"}"`);
}

console.log("\n=== ev_coverage_class");
const cls = ev.length === 0 ? "none" : ev.length < fuel.length / 2 ? "sparse" : "good";
console.log(`  ${ev.length} charging stations vs ${fuel.length} fuel -> ev_coverage_class=${cls}`);
const socket = ev.filter((p) => Object.keys(p.t).some((k) => k.startsWith("socket:")));
...
console.log("  tolls.est_band is retrieval (operator/authority site), not a fetch.");
