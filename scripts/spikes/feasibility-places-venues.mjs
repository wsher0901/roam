...
console.log(
  `generator: ${data.generator} | osm base: ${data.osm3s?.timestamp_osm_base}`,
);
...
for (const e of els) {
  ...
  else if (t.tourism === "attraction" || t.historic)
    type = "city walking & sightseeing";
...
console.log(
  ...
  JSON.stringify(
    (() => {
      const e =
        els.find((x) => x.tags?.name && x.tags?.tourism === "museum") ?? els[0];
      return {
        name: e.tags?.name,
