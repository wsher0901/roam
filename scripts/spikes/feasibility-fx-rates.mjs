...
console.log(
  "  covered   :",
  DEMO_CCY.filter((c) => rates[c] !== undefined).join(" "),
);
...
const f = await (
  await fetch("https://api.frankfurter.app/latest?from=EUR", {
...
console.log(
  ...
  f.base,
  "| currencies:",
  Object.keys(f.rates).length,
);
const sameSet =
  Object.keys(f.rates).sort().join() ===
  Object.keys(rates)
    .filter((c) => c !== "EUR")
    .sort()
    .join();
...
const h = await (
  await fetch(
    "https://api.frankfurter.app/2026-01-02..2026-01-10?from=EUR&to=USD",
    {
      headers: { "User-Agent": UA },
    },
  )
...
console.log(
  ...
  h.end_date,
  "| days:",
  Object.keys(h.rates ?? {}).length,
);
console.log(
  "  sample:",
  JSON.stringify(Object.entries(h.rates ?? {}).slice(0, 3)),
);

// --- 6. The one that was proposed and no longer works ---------------------
console.log("\n=== exchangerate.host (proposed candidate) — re-vetted live");
const x = await fetch("https://api.exchangerate.host/latest?base=EUR", {
  headers: { "User-Agent": UA },
});
console.log(
  "  HTTP",
  x.status,
  "->",
  (await x.text()).replace(/\s+/g, " ").slice(0, 140),
);
