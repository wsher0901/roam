...
## Status
Shipped — merged as
[PR #108](https://github.com/wsher0901/roam/pull/108) on the
founder's word after external review. Every verification green; no
deviations. This file is the first story welded THROUGH the
quadrant step it created (workshop/definition — a knowledge-system
convention; ties land here by law anyway).
...
(19) — with a [README legend](../../README.md) the weld reads
at move time (IS vs DOES picks the column, Roam-facing vs
builder-facing picks the row; ties → workshop/definition). The
memory [TEMPLATE](../../../memory/TEMPLATE.md) became the single
owner of the locked format and the post-D-032 Status vocabulary the
...
impossible rather than hand-checked; check-links (1270/0) confirms.
