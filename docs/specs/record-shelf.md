...
type: spec
id: record-shelf
title: The record shelf — docs/record/ is born, the frozen record in one place
status: open
stage: workshop
...
**Links:** decision minted on this bench (the next D-number; the
spec gains the anchor link the commit the entry exists) · queue:
the founder's 2026-07-27 three-bench mandate (bench 1 of 3).
