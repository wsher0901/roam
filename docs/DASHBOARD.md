...
Stamp: 2026-07-24 · 00:54 · ship · home PC
...
1. 🟡 Re-save the cockpit routine box from the FINAL charter
   master — once, covering both benches: rule 1 changed at
   [#213](https://github.com/wsher0901/roam/pull/213) (the
   self-seat duty), rule 5 at
   [#216](https://github.com/wsher0901/roam/pull/216) (the fleet
   table) (since 07-24).
   → [SETUP §cloud accounts](SETUP.md#once-and-done--cloud-accounts)
2. 🟡 Copy the new WEB-INSTRUCTIONS v5 master into the claude.ai
   Roam Project's instructions box (settings → project
   instructions — the box is a copy and still teaches v4)
   (since 07-24).
...
|---|---|---|---|---|
| main · control tower | — (response-doctrine shipped [#216](https://github.com/wsher0901/roam/pull/216); no bench open) | 🟢 | 00:54 (this repaint) | — |

↳ main micro: — (no active task)

Ground state, nothing in flight. FIVE benches welded this sitting
— the currency audit
...
LIVE TEST rides the next real flight by the founder's word), and
the response doctrine
([#216](https://github.com/wsher0901/roam/pull/216), D-052: one
standard for speaking to the founder; WEB-INSTRUCTIONS at v5).
The queue: THE CHRONICLE LAYER (its own chat, after the v5 paste
lands in the box).
...
[#216](https://github.com/wsher0901/roam/pull/216) (response
doctrine + v5, tower-authored) — is DONE, verdict PASS on
`5f4be89` (nine files, both HOME sections verified, the fleet
table verbatim, v5 with zero paste-block survivals, the gates
re-run independently), delivered inline with the founder's merge
word. Earlier reviews, all DONE:
...
[#195](https://github.com/wsher0901/roam/pull/195) → next: the
founder pastes the v5 master into the Project box (Needs-you 2);
grade the cockpit maiden, once the closeout bench opens; THE
CHRONICLE LAYER gets its own chat. Under the surface doctrine
...
diffs — v5 now says so in the box itself.
