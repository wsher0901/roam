...
One question the lane should expect to meet rather than dodge: the
list is headed "one-time per machine", and a cloud seat is born
fresh every flight. If the heading turns out to be the wrong frame
for this step, that is a finding for the memory — not a licence to
