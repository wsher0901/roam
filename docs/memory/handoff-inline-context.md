...
updated: 2026-07-15 · ship-to-gate · work PC
---
# handoff-inline-context — Handoff input inversion

## Status
complete, awaiting merge — bench, D-040 decide commit, and the full
ripple are on origin ([PR #126](https://github.com/wsher0901/roam/pull/126)).
Local CI mirror green; the PR is flipped ready and sits at THE GATE.
Not merged: founder approval follows external review in the Web chat.
...
- The claude.ai settings-box copy of WEB-INSTRUCTIONS goes stale once
  its footer changes here. The existing Needs-you item (re-paste the
  box) already covers this; the ship-tail repaint should refresh that
  line's wording. Flagged in the PR summary.
...
atomic weld — tick nothing on ROADMAP (workshop task), git mv this
memory to history/workshop/definition/, spec frontmatter → shipped,
ledger line, squash-merge.
...
(1488 links, 0 broken) proved the repoint.
...
this memory and the spec. The four live surfaces the rule actually
...
the ripple targets: [handoff §2](../skills/handoff.md#2--the-inline-webdesign-paste-full-only),
