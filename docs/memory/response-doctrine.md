...
(§Response doctrine, one home), the founder's command card, the
cockpit's fleet-table report, the three rituals' status
templates, and WEB-INSTRUCTIONS rewritten to v5 so the Web master
finally describes CC-direct instead of teaching every new chat
...
queue after the word: THE CHRONICLE LAYER (its own chat, the
founder's earlier word).

## The story

**Birth.** STEP 0 held twice over: origin heads = `main` only at
`4f8aec4`, and the self-seat gate grep found
[#213](https://github.com/wsher0901/roam/pull/213)'s weld in
place — the founder's interrupt-then-reorder of the two mandates
enforced itself exactly as designed.

**Placement calls, both one-line.** The doctrine and the card
landed as top-level HOME sections between §Reading the board and
...
**The rewrites, not annotations.** All three rituals' close
reports and briefs were rebuilt to the founder's four-part shape
(WORKING ON · WHAT HAPPENED · SUPPLEMENTAL · PARALLEL/RECENT
FLIGHTS). Pickup's old EXIT TABLE dissolved into
PARALLEL/RECENT FLIGHTS (per-item 🟢/⏸ with exact resume words —
...
**v5, and what died with v4.** The old master opened by calling
...
carries the doctrine compactly — a sanctioned restatement inside
the fenced box, since the box must work as a standalone paste;
the LIVING one-home stays HOME, every other prose home
referencing. "paste block" greps ZERO in the new file; the
master-copy pattern line survives, and the WEB HANDOFF block now
answers in the founder's own template with WHERE · WHAT · WHY
around it.
