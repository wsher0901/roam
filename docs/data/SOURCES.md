# Source registry — V1
One entry per source slot (FACTS.md "T2–T6 assignment map"). Each
entry records: facts served · chosen source · confirmed payload keys
(vs the FACTS.md Dictionary) · grade · freshness · coverage (D-010
...
(D-015): caching-prohibited sources disqualify a slot from the asset
layer. Grades are living (TELEMETRY.md §3 demotion law).

Grade scale (provisional until ratified in V1.S1.T7):
...
### weather-forecast
- Serves: WX-01, WX-02, WX-03, WX-04, WX-05, WX-06, WX-07, WX-08,
  WX-14 (and the forecast half of WX-10).
...
- Confirmed keys (spike, all present, zero missing): temperature_2m,
  ...
  SI-native (D-013 friendly).
- Grade: A. Forecast horizon: 7 days spiked; API supports up to 16
  (beyond horizon, WX-11 climatology takes over per FACTS).
...
- Alternatives rejected: OpenWeatherMap (free-tier data CC BY-SA —
  share-alike risk for the asset layer per D-015; re-verify if ever
...
### weather-climatology
- Serves: WX-11 (Suggest's merit workhorse); derivation input to
  WX-12.
...
- Confirmed keys (spike): daily temperature_2m_max, temperature_2m_min,
  ...
  us per date-of-year window (the computation is the WX-11 value; the
...
- Alternatives rejected: Meteostat (CC BY-NC — non-commercial-only
  kills product use; D-015 disqualifier); NOAA NCEI normals (US-only —
...
### snow-conditions
- Serves: WX-10 (snowfall + snow depth).
...
- Grade: B — global model values, reliable for "is it snowing /
  ...
  represented — per FACTS, model values render as labeled estimates
...
- Source: DERIVED + CURATED — no external fetch. Risk windows
  ...
  computed from WX-11 climatology statistics (e.g. months where
...
### activity-profiles
- Serves: WX-13 (weather-sensitivity profiles × 15 activity types).
- Source: CURATED IN-REPO data file — we are the source. Contents
  authored in V1.S3.T3; format fixed by FACTS.md; versioned in git
