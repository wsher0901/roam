...
One reading is asserted rather than ruled, and it is the bench's
whole design: **ruling 2 fixes when a story is OWED, not that a weld
must write one on the spot.** With 63 of 66 items unwritten, the
strict reading would turn every weld into a story-writing bench and
contradict the pilot's cost finding. The spec states it where the
...
THE SEQUENCING IS THE FOUNDER'S POINT AND IT IS A GOOD ONE. A fleet
of shelf benches would multiply welds against a rule nothing
enforces, so every story it wrote would start decaying immediately
and by exactly the mechanism the pilot just documented. Building the
enforcement first means the fleet's output stays true as it lands.

THE SWEEP RAN BEFORE THE NOT-A-DECISION NOTE WAS WRITTEN, because
...
THE DESIGN'S RISK IS SYMMETRIC, which is why the spec argues both
edges: too strict and every weld becomes a story bench; too loose
and endings rot as land's did. Three cases — incorporate into a
written story · update a census row's ending cell · add a row for an
item the 2026-07-27 census never saw — plus an honest escape when
nothing plausibly matches, because a ritual that forces a false
