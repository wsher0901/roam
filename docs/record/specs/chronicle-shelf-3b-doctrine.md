...
Write the **7 unwritten stories** of Shelf III slice B — laws & doctrine — the standing rules and the machinery that records them.
...
1. **LAWS — the always-loaded constitution** — `system piece`
2. **DECISIONS and decide** — `system piece`
3. **the routing law — one home per class** — `system piece`
4. **the retroactivity law and the currency sweeps** — `system piece`
5. **the corpus writing laws** — `system piece`
6. **the derivation law** — `system piece`
7. **the response doctrine** — `system piece`
...
- [ ] 7 story files exist in `docs/chronicle/`, one per roster
