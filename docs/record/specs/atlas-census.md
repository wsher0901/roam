...
**Links:** [ATLAS](../../ATLAS.md) (the page gaining a diagram) ·
[HOME](../../HOME.md) (one files-table row) · source: the
founder's approved mechanism census, Web ops chat 2026-07-31.

## Goal

[ATLAS](../../ATLAS.md) draws six things the workshop DOES — the
task loop, the file flow, away and return, a lane's life, the
...
machinery file itself, so a diagram can never become a second
source for a rule.

## Out of scope

- **Any master's fence**, [LAWS](../../LAWS.md), the skills, the
  board. Nothing there is edited; a swept citation only if the
  grep proves one.
- **The other six diagrams' content.** Only their Boxes lines
  gain citations (diagram 1's verification-loop home, diagrams 3
  and 6's baton case table).
- **New doctrine.** The diagram renders what the record already
  says; anything it cannot cite a prose home for does not go in.

## Plan

1. Bench birth: branch, this spec, memory stub, draft PR.
2. [decide](../../skills/decide.md) for D-071, short heading per
   [D-068](../DECISIONS.md#d-068--the-short-anchor-law).
3. [ATLAS](../../ATLAS.md): header six → seven; restamp from the
   shell clock; the new §7 with its mermaid flowchart and its
   dashed judgment-only note; its Boxes line, prose homes only.
4. Diagram 1's Boxes line: the verification loop cites
   [ship §1](../../skills/ship.md#1--preflight) as the commands'
   home, keeping LAWS §Workflow beside it as the law's home.
5. Diagrams 3 and 6: each Boxes line gains the baton case table
   at [handoff §4](../../skills/handoff.md).
6. [HOME](../../HOME.md): the files-table ATLAS row, six → seven.
7. THE SWEEP by destination-grep for "six diagrams" and any
   renamed anchor; near-zero expected, the change being additive.
8. Verification loop, full CI mirror, Actions green, the mermaid
   block confirmed to render, ship §6 critic; ship to THE GATE.
   Tower-authored — external Web review precedes the word.
...
None at birth. Where the census and the wiring disagree, the
wiring wins and the memory records the difference — a diagram
that draws an intention rather than a fact is worse than no
diagram.
